...
APP_MAINCLASS=net.itxiu.config.server.ConfigServerApplication
...
fork(){
  ...
  for i in {1..3}; do
    ...
    nohup $JAVA_HOME/bin/java $JAVA_OPTS -Dserver.port=$port -classpath $CLASSPATH $APP_MAINCLASS >$APP_HOME/log/console$port.log 2>&1 &
    port=$(($port+1))
  done
